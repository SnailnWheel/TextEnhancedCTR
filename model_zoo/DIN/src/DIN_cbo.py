...
class DIN_sub_1(BaseModel):
    def __init__(self, 
                 ...
                 **kwargs):
        super(DIN_sub_1, self).__init__(feature_map,
                                  model_id=model_id, 
                                  gpu=gpu, 
...
class DIN_cbo(BaseModel):
    def __init__(self, feature_map, **params):
        super(DIN_cbo, self).__init__(feature_map, **params)
        self.din_id = DIN_sub_1(feature_map, din_id="id", **params)
        self.din_text = DIN_sub_1(feature_map, din_id="text", **params)
        temp_input_dim = params["dnn_hidden_units"][-1] * 2
        self.output_layer = nn.Sequential(
            nn.Linear(temp_input_dim, 1, bias=True),
    ...
    def forward(self, inputs):
        ...
        output = self.output_layer(torch.cat([id_output, text_output], dim=-1))
