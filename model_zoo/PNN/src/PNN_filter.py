...
from fuxictr.pytorch.layers import FeatureEmbeddingDict, MLP_Block, InnerProductInteraction, MaskedAveragePooling, Sim_Score_Filter


class PNN_filter(BaseModel):
    def __init__(self, 
                 feature_map, 
                 model_id="PNN", 
                 gpu=-1, 
                 ...
                 **kwargs):
        super(PNN_filter, self).__init__(feature_map, 
                                  model_id=model_id, 
                                  gpu=gpu, 
        ...
        feature_map.num_fields = feature_map.get_num_fields(feature_source="id")
        ...
        input_dim = int(feature_map.num_fields * (feature_map.num_fields - 1) / 2) \
                  + feature_map.sum_emb_out_dim(feature_source="id")
